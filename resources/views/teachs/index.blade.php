...
<div class="pagetitle">
    <div class="row">
        ...
        <div class="col-sm-6">
           <a href="{{route('teachs.create')}}"> <button type="button" class="btn btn-gray float-right">Assign Subject</button></a>
        </div>
    </div>
</div>
            ...
            <div class="table-responsive">
                <table class="table" id="myTable">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{ $teach->getsubject->sub_name }}</td>
                            <td>{{ $teach->getsubject->sub_code }}</td>
                            <td>{{ $teach->getfaculty->name }}</td>
                            
                            <td>
                                <form method="POST" action="{{ route('teachs.destroy',$teach->id) }}">
                                    @method('DELETE')
                                    @csrf
                                    {{-- <a href="{{ route('subjects.edit',$teach->id ) }}"><button type="button" class="btn btn-warning btn-sm"><i class="bi bi-pencil-square"></i></button></a> --}}
